...
const NavbarAdmin = () => {
  ...
  const handleLogOut = async () => {
    try {
      ...
      navigate("/"); // kembali ke dashboard default
    } catch (error) {
  ...
  if (loading) {
    return (
      <nav className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-pink-300 via-pink-100 to-white shadow-md relative z-50 animate-pulse">
        <span className="text-pink-700 font-bold text-xl">Loading Admin Panel...</span>
        <div className="w-10 h-10 bg-pink-100 rounded-full"></div>
      </nav>
    );
  }
      ...
      <nav className="flex items-center justify-between px-6 py-0 bg-gradient-to-r from-pink-300 via-pink-100 to-white shadow-md relative z-50">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-pink-700 font-bold text-xl hidden md:block">Monmon's Hobbies Admin</span>
          </div>
        </div>
        ...
        <div className="relative">
          {isAuthenticated ? (
            <>
              <div
                className="w-10 h-10 bg-pink-100 rounded-full flex items-center justify-center cursor-pointer hover:bg-pink-200 transition-all duration-300 transform hover:scale-110 shadow-md z-50 overflow-hidden"
                  ...
                  <div className="absolute right-0 mt-3 w-72 bg-white rounded-2xl shadow-lg overflow-hidden z-50 border border-pink-200">
                    ...
                    <div className="p-4 space-y-2">
                      <button
                        ...
                        className="w-full text-left px-4 py-3 hover:bg-pink-50 rounded-xl transition-colors flex items-center space-x-3"
                      >
                      ...
                      <button
                        ...
                        className="w-full px-4 py-3 bg-gradient-to-r from-pink-400 to-pink-500 text-white rounded-xl hover:from-pink-500 hover:to-pink-600 transition-all duration-300 font-medium shadow-md transform hover:scale-105 flex items-center justify-center space-x-2"
                      >
            ...
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-pink-400 to-pink-500 text-white rounded-xl shadow-md hover:from-pink-500 hover:to-pink-600 transition-all"
            >
              Masuk
            </button>
          )}
        ...
        >
          <div className="max-w-7xl mx-auto py-10 px-6">
            <div className="mb-8 text-center">
              <h2 className="text-3xl font-bold bg-gradient-to-r from-pink-500 to-pink-400 bg-clip-text text-transparent mb-3">🎯 Admin Control Panel</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
              {navItems.filter(item => item.key !== 'add-product' && item.key !== 'dashboard-main').map(item => {
                ...
                return (
                  <button key={item.key} onClick={() => handleNavClick(item.path)} className={`group relative overflow-hidden rounded-3xl bg-white p-8 shadow-md hover:shadow-lg transition-all duration-500 transform hover:-translate-y-3 ${isActive ? "ring-4 ring-pink-300 scale-105" : ""}`}>
                    <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-br from-pink-100/40 to-pink-100/30 rounded-full -mr-20 -mt-20"></div>
                    <div className="absolute bottom-0 left-0 w-32 h-32 bg-gradient-to-tr from-pink-100/30 to-pink-50/20 rounded-full -ml-16 -mb-16"></div>
                    <div className="relative z-10">
                      <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 shadow-md ${isActive ? "bg-gradient-to-br from-pink-400 to-pink-300 text-white scale-110" : "bg-gradient-to-br from-pink-50 to-pink-50 text-pink-500 group-hover:from-pink-400 group-hover:to-pink-300 group-hover:text-white"}`}>
                        {item.icon}
                      </div>
                      <h3 className={`font-bold text-2xl mb-3 ${isActive ? "text-pink-600" : "text-gray-800 group-hover:text-pink-500"}`}>{item.name}</h3>
            ...
            <div className="flex justify-center">
              {(() => {
                ...
                const isActive = currentPath === item.path;
                return (
                  <button onClick={() => handleNavClick(item.path)} className={`group w-full md:w-3/4 lg:w-1/2 relative overflow-hidden rounded-3xl bg-white p-10 shadow-md hover:shadow-lg transition-all duration-500 transform hover:-translate-y-3 ${isActive ? "ring-4 ring-pink-300 scale-105" : ""}`}>
                    ...
                    <div className="relative z-10 flex flex-col items-center text-center">
                      <div className={`w-24 h-24 rounded-3xl flex items-center justify-center mb-6 shadow-md ${isActive ? "bg-gradient-to-br from-pink-400 to-pink-300 text-white scale-110" : "bg-gradient-to-br from-pink-50 to-pink-50 text-pink-500 group-hover:from-pink-400 group-hover:to-pink-300 group-hover:text-white"}`}>
                        {item.icon}
                      </div>
                      <h3 className={`font-bold text-3xl mb-3 ${isActive ? "text-pink-600" : "text-gray-800 group-hover:text-pink-500"}`}>{item.name}</h3>
