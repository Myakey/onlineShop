      ...
      <nav className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-pink-200 via-cyan-100 to-white shadow-md relative z-50">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2" onClick={goToHome} style={{cursor: 'pointer'}}>
            ...
            <span className="text-pink-700 font-bold text-xl hidden md:block select-none">Ambalabus</span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          {loading ? (
            ...
            <div className="relative">
              <div className="w-10 h-10 bg-cyan-100 rounded-full flex items-center justify-center cursor-pointer hover:bg-cyan-200 transition-all duration-300 transform hover:scale-110 shadow-md z-50 overflow-hidden" onClick={toggleProfile}>
                ...
                <>
                  <div className="absolute right-0 mt-3 w-72 bg-white rounded-2xl shadow-lg overflow-hidden z-50 border border-cyan-200">
                    ...
                    <div className="p-4 space-y-2">
                      <button onClick={goToProfile} className="w-full text-left px-4 py-3 hover:bg-cyan-50 rounded-xl transition-colors flex items-center space-x-3">
                        <User className="w-5 h-5 text-cyan-500" />
                        <span className="text-gray-700 font-medium">Edit Profil</span>
                      </button>

                      {!isAdmin && (
                        <button onClick={() => navigate("/order-list")} className="w-full text-left px-4 py-3 hover:bg-cyan-50 rounded-xl transition-colors flex items-center space-x-3">
                      ...
                      <button onClick={handleLogOut} className="w-full px-4 py-3 bg-gradient-to-r from-pink-400 to-cyan-400 text-white rounded-xl hover:from-pink-500 hover:to-cyan-500 transition-all duration-300 font-medium shadow-md transform hover:scale-105">
                        Keluar
                      </button>
                    </div>
                  </div>
            ...
            <button onClick={goToLogin} className="flex items-center space-x-2 px-6 py-2.5 bg-gradient-to-r from-pink-400 to-cyan-400 text-white rounded-full hover:from-pink-500 hover:to-cyan-500 transition-all duration-300 font-medium shadow-md transform hover:scale-105">
      ...
      <div className={`absolute top-full left-0 right-0 bg-[#FFF5FA] shadow-xl border-t-4 border-pink-200 transition-all duration-500 ease-in-out transform z-40 ${isMenuOpen ? "opacity-100 translate-y-0 visible" : "opacity-0 -translate-y-8 invisible"}`}>
        <div className="max-w-7xl mx-auto py-10 px-6">
          <div className="mb-8 text-center">
            <h2 className="text-3xl font-bold bg-gradient-to-r from-pink-500 to-cyan-400 bg-clip-text text-transparent mb-3">{isAdmin ? "🎯 Admin Control Panel" : "🍽️ Jelajahi Menu Kami"}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
            {navItems.filter(item => item.key !== "add-product").map(item => (
              <button key={item.key} onClick={() => handleNavClick(item.path, item.key)} className={`group relative overflow-hidden rounded-3xl bg-white p-8 shadow-md hover:shadow-lg transition-all duration-500 transform hover:-translate-y-3 ${currentPageState.key === item.key ? "ring-4 ring-cyan-300 scale-105" : ""}`}>
                ...
                <div className="relative z-10 text-left">
                  <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 transition-all duration-500 shadow-md ${currentPageState.key === item.key ? "bg-gradient-to-br from-pink-400 to-cyan-300 text-white scale-110" : "bg-gradient-to-br from-pink-50 to-cyan-50 text-pink-500 group-hover:from-pink-400 group-hover:to-cyan-300 group-hover:text-white group-hover:scale-110"}`}>{getIcon(item.key)}</div>
                  <h3 className={`font-bold text-2xl mb-3 transition-colors duration-300 ${currentPageState.key === item.key ? "text-pink-600" : "text-gray-800 group-hover:text-cyan-500"}`}>{item.name}</h3>
          ...
          {isAdmin && (
            <div className="flex justify-center mb-8">
              <button onClick={() => handleNavClick("/admin/add-product", "add-product")} className={`group w-full md:w-3/4 lg:w-1/2 relative overflow-hidden rounded-3xl bg-white p-10 shadow-md hover:shadow-lg transition-all duration-500 transform hover:-translate-y-3 ${currentPageState.key === "add-product" ? "ring-4 ring-cyan-300 scale-105" : ""}`}>
                ...
                <div className="relative z-10 flex flex-col items-center text-center">
                  <div className={`w-24 h-24 rounded-3xl flex items-center justify-center mb-6 shadow-md ${currentPageState.key === "add-product" ? "bg-gradient-to-br from-pink-400 to-cyan-300 text-white scale-110" : "bg-gradient-to-br from-pink-50 to-cyan-50 text-pink-500 group-hover:from-pink-400 group-hover:to-cyan-300 group-hover:text-white"}`}><Plus className="w-10 h-10" /></div>
                  <h3 className={`font-bold text-3xl mb-3 ${currentPageState.key === "add-product" ? "text-pink-600" : "text-gray-800 group-hover:text-cyan-500"}`}>Tambah Produk</h3>
